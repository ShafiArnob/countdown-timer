...
export default function SubscriptionForm() {
  ...
  const handleSubmit = (e) => {
    ...
    //http://103.108.146.90:5000/sendemail
    fetch('http://localhost:5000/sendemail', {
      ...
      .then(data => hitToast( data.success ? 'success' : 'error' ,data.message))//*fixed 
      .catch(() => hitToast('error', 'Something went wrong. Please try again.')) //*fixed

    setAlertClass('');
  }

  const validate = (email) => { 
    if (email.trim().match(/^([a-zA-Z0-9_\-.]+)@((\[[0-9]{1,3}\.[0-9]{1,3}\.[0-9]{1,3}\.)|(([a-zA-Z0-9-]+\.)+))([a-zA-Z]{1,5}|[0-9]{1,3})(\]?)$/) === null) { //* Fixed
      return false;
    } else if (email.trim() === '') {
      return false;
    }
